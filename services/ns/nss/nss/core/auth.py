...
async def require_auth(request: StarletteRequest) -> AuthContext:
...
def require_founder(request: StarletteRequest):
    try:
        return require_ops_key(request.headers.get("X-NS-OPS-KEY"))
    except TypeError:
        return require_ops_key(x_ns_ops_key=request.headers.get("X-NS-OPS-KEY"))


def require_permission(perm: str):
    """FastAPI dependency factory — checks specific permission."""
    async def _dep(request: StarletteRequest) -> AuthContext:
        from fastapi import HTTPException
...
def require_ops_key(
    x_ns_ops_key: str | None = Header(default=None, alias="X-NS-OPS-KEY"),
):
    import os
    expected = os.getenv("NS_OPS_KEY", "")
    if not expected:
        raise HTTPException(status_code=500, detail="NS_OPS_KEY not set")
    if not x_ns_ops_key or x_ns_ops_key != expected:
        raise HTTPException(status_code=401, detail="Unauthorized")
    try:
        return AuthContext(role="OPS")
    except Exception:
        return {"role": "OPS"}
