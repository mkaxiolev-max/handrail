...
# ─── SSD Session Persistence ─────────────────────────────────────────────────
_SSD_SESSIONS_DIR = Path("/Volumes/NSExternal/ALEXANDRIA/sessions")

def _ssd_session_path(call_sid: str) -> Path:
    return _SSD_SESSIONS_DIR / f"{call_sid}.json"

def _persist_session(session: "VoiceSession") -> None:
    """Write session state to SSD. Idempotent — always overwrites with latest."""
    try:
        _SSD_SESSIONS_DIR.mkdir(parents=True, exist_ok=True)
        _ssd_session_path(session.call_sid).write_text(
            json.dumps(session.to_dict(), default=str)
        )
    except Exception:
        pass

def load_persisted_sessions(max_age_hours: int = 24) -> Dict[str, "VoiceSession"]:
    """Load sessions from SSD written within the last max_age_hours. Returns call_sid → session."""
    loaded: Dict[str, "VoiceSession"] = {}
    if not _SSD_SESSIONS_DIR.exists():
        return loaded
    cutoff = datetime.now(timezone.utc).timestamp() - max_age_hours * 3600
    for f in _SSD_SESSIONS_DIR.glob("*.json"):
        try:
            data = json.loads(f.read_text())
            # Skip closed sessions (ended_at set) and stale ones
            ...
            session = VoiceSession(
                ...
                tier=data.get("tier", TIER_E),
                mode=data.get("mode", DEFAULT_MODE),
            )
            session.call_id = data["call_id"]
            session.started_at = data["started_at"]
            session.turns = data.get("turns", [])
            session.pending_actions = data.get("pending_actions", [])
            session.recording_urls = data.get("recording_urls", [])
            loaded[call_sid] = session
        except Exception:
            pass
    return loaded
