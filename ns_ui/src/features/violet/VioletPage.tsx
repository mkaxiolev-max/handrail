    ...
    <div style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', marginBottom: 10 }}>
      {!isUser && (
        <div style={{
          ...
          fontSize: 11, color: '#fff', fontWeight: 'bold', flexShrink: 0, marginRight: 8, marginTop: 2,
        }}>V</div>
      )}
      <div style={{
        maxWidth: '72%',
        ...
        padding: '8px 12px', fontSize: 12, color: isUser ? '#c7d2fe' : '#e0e7ff',
        lineHeight: 1.55, wordBreak: 'break-word',
      }}>
        {msg.text}
        <div style={{ fontSize: 9, color: '#6366f1', marginTop: 4, opacity: 0.7 }}>{msg.ts}</div>
      </div>
    </div>
  )
}

export function VioletPage() {
  ...
  const send = async (text?: string) => {
    ...
    setLoading(true)
    try {
      const data = await api.post<{ ok: boolean; text?: string; provider?: string; error?: string }>(
        EP.violet.chat(), { message: prompt }
      )
      if (data.ok) {
        setProvider(data.provider ?? null)
        setMessages(prev => [...prev, { role: 'violet', text: data.text ?? '', ts: new Date().toLocaleTimeString() }])
      } else {
        setMessages(prev => [...prev, { role: 'violet', text: `[Error] ${data.error ?? 'Unknown error'}`, ts: new Date().toLocaleTimeString() }])
      }
    } catch {
      setMessages(prev => [...prev, { role: 'violet', text: '[Network error] Cannot reach NS Core.', ts: new Date().toLocaleTimeString() }])
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{
      ...
      background: '#0d0d1f', borderRadius: 10, border: `1px solid #2d2a6e`, overflow: 'hidden',
    }}>
      {/* Header */}
      <div style={{
        padding: '10px 16px', background: '#12103a', borderBottom: '1px solid #2d2a6e',
        ...
        <div style={{ padding: '0 12px 8px', display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {SUGGESTIONS.map(s => (
            <button key={s} onClick={() => send(s)} style={{
              ...
              borderRadius: 20, color: '#a5b4fc', cursor: 'pointer', fontFamily: 'monospace',
            }}>{s}</button>
          ))}
        </div>
      )}
      ...
      {/* Input */}
      <div style={{
        padding: '10px 12px', borderTop: '1px solid #2d2a6e', background: '#12103a',
        ...
        <textarea
          ...
          style={{
            ...
            fontSize: 12, fontFamily: 'monospace', resize: 'none', outline: 'none', lineHeight: 1.5,
          }}
        />
        <button
          onClick={() => send()}
