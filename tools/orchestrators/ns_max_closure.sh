...
# Theoretical ceilings (per-instrument) — source of truth for gap calc
...
# Ordered list of phases (bash 3.2 — plain space-separated string)
...
# git_c — commit with axiolevns identity (no global git config mutation)
git_c() {
  git -C "$REPO_DIR" \
...
# receipt — write Lineage Fabric JSON receipt to Alexandria
receipt() {
...
# pytest_retry — phase-gated pytest with up-to-N retries and fix-request emission
# Usage: pytest_retry <phase-id> <pytest-args...>
pytest_retry() {
  local id="$1"; shift
  local attempt=1
...
# vitest_run — single-shot vitest wrapper
vitest_run() {
  local id="$1"
...
# claude_prompt — write a PROMPT.md and invoke the Claude CLI for a phase
# Usage: claude_prompt <id> <prompt-body>
claude_prompt() {
  local id="$1"; shift
  local body="$*"
  local pdir="$ART_DIR/prompts"; mkdir -p "$pdir"
  local pfile="$pdir/${id}.PROMPT.md"
  printf "%s\n" "$body" > "$pfile"
  if have "$CLAUDE_BIN"; then
    info "$id invoking claude CLI with $pfile"
    ( cd "$REPO_DIR" && "$CLAUDE_BIN" --model "$CLAUDE_MODEL" --prompt-file "$pfile" ) \
...
# artifact_write — emit markdown + JSON artifact pair
artifact_write() {
...
# safe_tag — create an annotated tag idempotently
safe_tag() {
...
# yubikey_check <mode>  mode ∈ warn|hard
yubikey_check() {
  local mode="${1:-warn}"
  if have ykman; then
...
# simple floating-point add/mul via awk (bash 3.2 has no FP)
...
# -----------------------------------------------------------------------------
# ONTOLOGY GUARD — deprecated-name scan
# -----------------------------------------------------------------------------
ontology_guard() {
  local id="$1"
...
# ---------- M0 SECURITY REMEDIATION GATE -------------------------------------
do_m0() {
  info "M0 security remediation gate"
  # 0 active tracked secrets invariant
...
HOOK
  ...
  # Optional filter-repo history rewrite
  if [ "$AXIOLEV_SKIP_FILTER_REPO" = "0" ]; then
    ...
    info "(skipped in unattended mode; set AXIOLEV_SKIP_FILTER_REPO=0 with TTY to enable)"
  else
...
# ---------- M1 INS-06 CALIBRATION SFT ----------------------------------------
do_m1() {
  info "M1 INS-06 Calibration SFT — Brier/ECE/AURC + Chow selective prediction"
  mkdir -p "$REPO_DIR/services/calibration"
  local pfile
  pfile=$(claude_prompt "M1" "$(cat <<'PROMPT'
...
4. services/calibration/selective.py — Chow's rule selective prediction with
   a learned threshold τ chosen to minimize risk at target coverage c.
5. services/calibration/pipeline.py — end-to-end runner that logs per-epoch
   {brier, ece, aurc, τ, coverage, risk} to Alexandria under
...
6. Update tests/test_calibration.py to exercise all four modules; add
   ...
   - SFT convergence on a miscalibrated softmax (temperature T→1 ±0.05)
...
PROMPT
  ...
  git_c add services/calibration tests/test_calibration.py tests/test_calibration_sft.py
  ...
  artifact_write "M1" \
    "# M1 INS-06 Calibration SFT — GREEN\nExpected credits: I2.calibration +7 → I2 ≈ 90.8" \
    "{\"phase\":\"M1\",\"insight\":\"INS-06\",\"credits\":{\"I2.calibration\":7}}"
}

# ---------- M2 INS-03 REVERSIBILITY 100% -------------------------------------
do_m2() {
  info "M2 INS-03 Reversibility registry — 80% → 100%"
  mkdir -p "$REPO_DIR/services/ns_core/reversibility"
  claude_prompt "M2" "$(cat <<'PROMPT'
...
# ---------- M3 INS-01 CPS ACTION-SURFACE EXPANSION ---------------------------
do_m3() {
  info "M3 INS-01 Full action-surface CPS expansion (R0–R4 tiering)"
  claude_prompt "M3" "$(cat <<'PROMPT'
...
2. services/hormetic/classifier.py — given a sweep trajectory, emit one of:
     Super-Gnoseogenic  (net knowledge gain > threshold, drift≤ε)
     Generative         (gain present, drift bounded)
     Plastic            (recovery only, no gain)
     Fragile            (post-sweep drift > ε)  — fails the phase.
...
- All sweep receipts → Lineage Fabric.
PROMPT
)"
...
# ---------- M5 INS-02 NVIR GENERATOR + ORACLE VALIDATOR ----------------------
do_m5() {
  yubikey_check hard
  info "M5 INS-02 NVIR generator-stack + oracle validator per domain"
  claude_prompt "M5" "$(cat <<'PROMPT'
...
- Receipts → Lineage Fabric under ledger/nvir/.
PROMPT
)"
  ...
  artifact_write "M5" \
    "# M5 INS-02 NVIR Complete — GREEN\nRemaining credits: I2+0.8 (on top of prior +5.94)" \
    "{\"phase\":\"M5\",\"insight\":\"INS-02\",\"credits\":{\"I2.remaining\":0.8}}"
}

# ---------- M6 INS-04 TLA+/APALACHE INVARIANT MODEL-CHECK --------------------
do_m6() {
  info "M6 INS-04 TLA+/Apalache bounded model-check on Dignity Kernel"
  mkdir -p "$REPO_DIR/specs/tla"
  claude_prompt "M6" "$(cat <<'PROMPT'
...
PROMPT
  ...
  # Best-effort apalache invocation
...
# ---------- M7 INS-05 VALIDATOR ADAPTERS -------------------------------------
do_m7() {
  info "M7 INS-05 Validator adapters (Lean / DFT-stub / FDA-class)"
  mkdir -p "$REPO_DIR/services/validators"
  claude_prompt "M7" "$(cat <<'PROMPT'
...
5. services/validators/registry.py — domain → adapter dispatch.
...
# ---------- M8 NS-AL PROOF-CARRYING EXECUTION --------------------------------
do_m8() {
  yubikey_check hard
  info "M8 NS-AL Proof-carrying execution — Constitutional Invariant 11"
  mkdir -p "$REPO_DIR/services/assurance"
  claude_prompt "M8" "$(cat <<'PROMPT'
...
2. services/assurance/dispatcher.py — verification dispatcher: given a
   ...
   verifier (Lean for proofs, SMT for bounded claims, validator adapters
   from INS-05 for domain claims); produces a VerificationReceipt or an
   ObligationArtifact if verification cannot be completed now.
...
4. tests/test_assurance_T070.py .. tests/test_assurance_T079.py — exactly
   10 new tests (T-070..T-079), each exercising one facet of Invariant 11:
     T-070 reject transition w/o receipt
     T-071 reject receipt w/o justification
     T-072 reject justification w/o verification-or-obligation
     T-073 accept verified receipt end-to-end
     T-074 obligation path with compensating action resolves cleanly
     T-075 contract precondition failure → rejection
     T-076 postcondition failure → rejection
...
PROMPT
  ...
  artifact_write "M9" \
    "# M9 I7 Category Tests — GREEN\nI7 target: 84.0 → 98.0 (post scoring upgrade)" \
    "{\"phase\":\"M9\",\"insight\":\"I7_TESTS\",\"i7_target\":98.0}"
}

# ---------- M10 MASTER v3.2 SCORER UPGRADE -----------------------------------
do_m10() {
  info "M10 MASTER v3.2 scorer — add I7 @ 0.10, recompute all versions"
  mkdir -p "$REPO_DIR/tools/scoring"
  claude_prompt "M10" "$(cat <<'PROMPT'
...
5. pytest must cover at least: weight-sum invariants (v3.1 Σw=1, v3.2 Σw=1),
...
# ---------- M12 SWIFT TEST TARGET WIRE-UP (BEST-EFFORT) ----------------------
do_m12() {
...
do_m13() {
  ...
  # Commit secret audit
  git -C "$REPO_DIR" log --all -p | grep -E "(BEGIN (OPENSSH|RSA|EC) PRIVATE|AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{36})" \
...
# ---------- M14 CERTIFICATION CEREMONY (GATED) -------------------------------
do_m14() {
...
# =============================================================================
# FINAL SUMMARY / FORWARD-LOOKING VERIFICATION
# =============================================================================
...
PER-INSTRUMENT (current live → ceiling)
  I1 Super-Omega v2:             88.02 → $CEIL_I1
  I2 Omega Intelligence v2:      88.36 → $CEIL_I2
  I3 UOIE v2 (admin-capped):     88.84 → $CEIL_I3
  I4 GPX-Ω:                      95.28 → $CEIL_I4
  I5 SAQ:                        89.00 → $CEIL_I5
  I6 Ω-Logos:                    88.00 → $CEIL_I6
  I7 Cert Power Score (v3.2):    84.00 → $CEIL_I7
