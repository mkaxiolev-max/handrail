...
# AXIOLEV Holdings LLC — NS∞ master ceremony sign script
...
TAG="ns-infinity-master-v1-${TS}"

echo "=== NS∞ Ceremony: MASTER-CERT — 220/220 — 18/18 gates — 2-of-2 quorum ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "MASTER-CERT — 220/220 — 18/18 gates — 2-of-2 quorum — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
body = {
    ...
    "tier": "MASTER-CERT — 220/220 — 18/18 gates — 2-of-2 quorum",
    "tag": "$TAG",
...
echo ""
echo "=== CEREMONY COMPLETE: MASTER-CERT — 220/220 — 18/18 gates — 2-of-2 quorum ==="
