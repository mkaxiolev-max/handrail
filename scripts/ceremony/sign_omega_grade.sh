...
# AXIOLEV Holdings LLC — NS∞ omega_grade ceremony sign script
...
TAG="ns-infinity-omega-grade-${TS}"

echo "=== NS∞ Ceremony: Omega-Grade — 160/220 milestone ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "Omega-Grade — 160/220 milestone — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
body = {
    ...
    "tier": "Omega-Grade — 160/220 milestone",
    "tag": "$TAG",
...
echo ""
echo "=== CEREMONY COMPLETE: Omega-Grade — 160/220 milestone ==="
