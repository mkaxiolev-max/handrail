...
# AXIOLEV Holdings LLC — NS∞ akashic ceremony sign script
...
TAG="ns-infinity-akashic-v1-${TS}"

echo "=== NS∞ Ceremony: Akashic — dim V complete ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "Akashic — dim V complete — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
echo ""
echo "=== CEREMONY COMPLETE: Akashic — dim V complete ==="
