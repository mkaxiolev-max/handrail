...
# AXIOLEV Holdings LLC — NS∞ governance ceremony sign script
...
TAG="ns-infinity-prime-p17-${TS}"

echo "=== NS∞ Ceremony: Governance-Bearing tier ceremony ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "Governance-Bearing tier ceremony — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
body = {
    ...
    "tier": "Governance-Bearing tier ceremony",
    "tag": "$TAG",
...
echo ""
echo "=== CEREMONY COMPLETE: Governance-Bearing tier ceremony ==="
