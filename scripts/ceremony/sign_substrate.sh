...
# AXIOLEV Holdings LLC — NS∞ substrate ceremony sign script
...
TAG="ns-infinity-prime-p16-${TS}"

echo "=== NS∞ Ceremony: Substrate-Strong ceremony — pubkey export, cert bundle dry-run ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "Substrate-Strong ceremony — pubkey export, cert bundle dry-run — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
body = {
    ...
    "tier": "Substrate-Strong ceremony — pubkey export, cert bundle dry-run",
    "tag": "$TAG",
...
echo ""
echo "=== CEREMONY COMPLETE: Substrate-Strong ceremony — pubkey export, cert bundle dry-run ==="
