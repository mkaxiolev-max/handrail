...
# AXIOLEV Holdings LLC — NS∞ high_assurance ceremony sign script
...
TAG="ns-infinity-high-assurance-${TS}"

echo "=== NS∞ Ceremony: High-Assurance — 120/220 milestone ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "High-Assurance — 120/220 milestone — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
body = {
    ...
    "tier": "High-Assurance — 120/220 milestone",
    "tag": "$TAG",
...
echo ""
echo "=== CEREMONY COMPLETE: High-Assurance — 120/220 milestone ==="
