...
# AXIOLEV Holdings LLC — NS∞ qr ceremony sign script
...
TAG="ns-infinity-quantum-ready-v1-${TS}"

echo "=== NS∞ Ceremony: Quantum-Ready — dim U complete ==="
...
cd "$RUN"
git -c user.name=axiolevns -c user.email=axiolevns@axiolev.com   tag -a "$TAG" -m "Quantum-Ready — dim U complete — composite=$COMPOSITE/220"   && echo "  ✓ tag $TAG applied (unsigned — sign manually with: git tag -s $TAG)"   || { echo "ERROR: tagging failed"; exit 1; }
...
body = {
    ...
    "tier": "Quantum-Ready — dim U complete",
    "tag": "$TAG",
...
echo ""
echo "=== CEREMONY COMPLETE: Quantum-Ready — dim U complete ==="
